  ...
  createSuggestionCard(suggestion) {
    ...
    card.innerHTML = `
      <div class="flex items-start justify-between mb-2">
        ...
        <span class="text-xs px-2 py-1 rounded-full bg-pink-50 text-primary-700">${this.getTypeLabel(suggestion.type)}</span>
      </div>
      ...
      <div class="flex gap-2">
        ${suggestion.onApply ? `
          <button class="apply-btn flex-1 px-3 py-1.5 bg-primary-600 text-white text-sm rounded-lg hover:bg-accent-500 transition-colors">
            Apply
          </button>
        ` : ''}
