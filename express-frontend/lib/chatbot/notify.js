const { LEAD_NOTIFY_TO } = require('./config');
...
async function sendViaResend(lead, apiKey) {
...
async function sendViaSendGrid(lead, apiKey) {
  const { subject, text } = buildBody(lead);
  const from = process.env.EMAIL_FROM_ADDRESS || 'contact@prooftamil.com';
  const fromName = process.env.EMAIL_FROM_NAME || 'ProofTamil';
...
async function notifyNewLead(lead) {
  ...
  try {
    ...
    console.error('[leads] notification transport threw:', error.message);
    return 'failed';
  }
