const { RAG_MAX_CONTEXT_CHARS, SITE_ORIGIN } = require('./config');
...
const PERSONA = `You are ProofBot, the assistant on ProofTamil (${SITE_ORIGIN}) — an AI writing platform for Tamil.
...
function truncateContext(chunks) {
  const kept = [];
...
function buildSystemPrompt({ chunks, pageUrl, locale }) {
