...
// biting us: one transient Gemini 429/500/timeout on a single strip
// killed the whole 6-strip transcription and the user lost everything
// the other 5 strips had already extracted. Now the caller sees which
// strips failed and decides whether to keep going or hard-fail.
...
async function runPipeline(input, opts) {
  ...
  // First pass — parallel transcribe. pickKey(opts, idx) rotates through
  // opts.apiKeys so strip i uses key i mod N (each strip lands on its own
  // key, avoiding single-key rate limits).
  ...
  // Retry any strip that errored, once, sequentially, on a DIFFERENT key.
  // Transient Gemini 429s / 500s / connection resets clear on a re-request
  // in the vast majority of cases; retrying only the failed strips (rather
  // than making the user re-upload the whole page) is much cheaper on wall
  // clock and on their monthly quota. Shifting the key by (i + N) means
  // retry never hits the same key that just failed.
  ...
  for (const i of failedIdx) {
    try {
      stripResults[i] = await transcribeBuffer(cut.strips[i].buffer, {
        model: opts.model,
        apiKey: pickKey(opts, i + cut.strips.length + 1),
        mimeType: 'image/png',
        timeoutMs: opts.timeoutMs,
      });
      retriedIdx.push(i);
    } catch (err) {
      // Keep the original error record; surfaced as a warning in meta
  ...
  // Only hard-fail if EVERY strip errored — otherwise the user gets useful
  // text for the strips that worked plus a warning listing the ones that
  // didn't. Failing on ANY error (the old behaviour) was biting users on
  // multi-strip pages where any single transient error killed the whole run.
