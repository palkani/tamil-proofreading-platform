...
// CreateSubmissionRequest is the body for POST /api/v1/submissions (e.g. save AI Content Writer output as draft).
...
// CreateSubmission creates a new draft (submission) without running proofreading.
// Used by AI Content Writer "Save to My Drafts". The draft appears in My Drafts and can be opened in Workspace.
func (h *Handlers) CreateSubmission(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "original_text is required"})
		return
	}

	originalText := strings.TrimSpace(req.OriginalText)
	if originalText == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "original_text cannot be empty"})
		return
	}
	...
	wordCount := h.nlpService.CountWords(originalText)
	if wordCount <= 0 {
		wordCount = 1
	}
	...
	submission := &models.Submission{
		...
		OriginalHTML:  strings.TrimSpace(req.OriginalHTML),
		WordCount:     wordCount,
	...
	if err := h.db.Create(submission).Error; err != nil {
		log.Printf("[SUBMISSION] CreateSubmission failed: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to save draft"})
		return
	}

	log.Printf("[SUBMISSION] Created draft (AI Content Writer) id=%d user_id=%d title=%q", submission.ID, userID, title)
	c.JSON(http.StatusCreated, gin.H{"success": true, "submission": submission})
}
