...
// Uses the file-based suggest engine (lexicon), not DB or TamilWordCache.
// GET /api/v1/tamil-words/autocomplete?query=a&limit=5
func (h *Handlers) AutocompleteTamil(c *gin.Context) {
	...
	if engine != nil {
		...
		defer cancel()
		out, err := engine.Suggest(ctx, suggest.SuggestRequest{Query: query, Limit: limit})
		if err == nil && len(out.Suggestions) > 0 {
			suggestions = make([]TamilSuggestion, 0, len(out.Suggestions))
			for _, s := range out.Suggestions {
				suggestions = append(suggestions, TamilSuggestion{
					TamilText:       s.Text,
					Transliteration: s.Latin,
					Frequency:       0,
					Category:        "",
				})
			}
