...
**This code now lives in the Tamil Proofreading Platform backend repo.** The ProofTamilRunner app has been retired as a separate repository; all code is under `backend/prooftamil-runner/`. You can run it from here (see Local run below) or deploy it as part of the same backend deployment (e.g. same Docker image or sidecar).
