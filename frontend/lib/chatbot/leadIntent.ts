...
export function answerSignalsUnknown(answer: string): boolean {
  return UNKNOWN_ANSWER.some((pattern) => pattern.test(answer));
}

export type LeadReason = 'buying-intent' | 'contact-request' | 'unanswered' | null;
...
export interface LeadIntentInput {
  ...
  message: string;
  /** How many chunks cleared the relevance floor. Zero means we could not answer. */
  retrievedCount: number;
  /** Whether the card has already been offered in this session. */
  alreadyOffered: boolean;
  /**
   * The assistant's completed reply, when available. Passed from the API route
   * after streaming finishes, since the `meta` line is emitted last.
   */
  answer?: string;
}
